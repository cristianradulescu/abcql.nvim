...
describe("Parser", function()
  ...
  describe("last_joined_table", function()
    it("returns the last joined table and its alias", function()
      local j = Parser.last_joined_table("SELECT * FROM a x JOIN b y ON x.i = y.i LEFT JOIN shop.`c` AS z ON ")
      assert.are.equal("c", j.table)
      assert.are.equal("shop", j.database)
      assert.are.equal("z", j.alias)
    end)

    it("has no alias when the next word is a keyword or missing", function()
      assert.is_nil(Parser.last_joined_table("SELECT * FROM a JOIN b ON ").alias)
      assert.is_nil(Parser.last_joined_table("SELECT * FROM a JOIN b").alias)
      assert.is_nil(Parser.last_joined_table("SELECT * FROM a"))
    end)
  end)
end)
